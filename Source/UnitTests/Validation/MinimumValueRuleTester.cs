...
{
	public static class MinimumValueRuleTester
	{
		[TestFixture]
		public class When_testing_value_that_is_not_too_small
		{
			...
			{
				Assert.That(new MinimumValueRule<string, int>(1, "Error", 2).Validate(), Is.Empty);
			}
		}
		...
		public class When_testing_value_that_is_too_small
		{
			...
			{
				Assert.That(new MinimumValueRule<string, int>(1, "Error", 0).Validate().ToArray(), Is.EquivalentTo(new[] { "Error" }));
			}
		}
	}
}
...
{
	public static class MaximumValueRuleTester
	{
		[TestFixture]
		public class When_testing_value_that_is_not_too_large
		{
			...
			{
				Assert.That(new MaximumValueRule<string, int>(1, "Error", 0).Validate(), Is.Empty);
			}
		}
		...
		public class When_testing_value_that_is_too_large
		{
			...
			{
				Assert.That(new MaximumValueRule<string, int>(1, "Error", 2).Validate().ToArray(), Is.EquivalentTo(new[] { "Error" }));
			}
		}
	}
}